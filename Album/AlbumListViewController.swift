...
class AlbumListViewController: UIViewController, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        var httpConnection = HttpConnection(url: "https://itunes.apple.com/search?term=%@&USA&media=music&entity=album&limit=10&sort=recent")
        self.itunesApiDao = ItunesAPIDao(httpConnection: httpConnection)
        self.albumSort = AlbumSort()
        
        self.spindle .startAnimating()
        
        var getAlbum = GetAlbumInteractor(albumDao: self.itunesApiDao!, sorter: self.albumSort!)
        
        var getAlbumRequest = GetAlbumRequest()
        ...
        getAlbumRequest.setCallback({(albumResponse) in
            println(albumResponse.getAlbums())
    ...
    func setupScrollView() {
        ...
        for i=0; i<self.albums.count; i++ {
            ...
            var imageView = UIImageView(frame: CGRectMake(CGFloat(i)*pagesScrollViewSize.width, 0, pagesScrollViewSize.width, pagesScrollViewSize.height))
